...
contract WebQDeployer is Ownable, ERC721SeaDropStructsErrorsAndEvents {
    ...
    event WebQDeployed(address webQ, address foundation, address NFT, address spNFT);

    constructor(address seaDropImplementation, 
                address payable safeImplementation,
                address nftSigner,
        ...
        address safe = address(new GnosisSafeProxy(safeImplementation));

        webQBase= payable(new WebQ(payable(safe)));

        Safe(safe).setup(safeOwners, safeThreshold, address(0), "", address(0), address(0), 0, payable(0));
        ...
        emit WebQDeployed(address(webQBase), safe, address(NFT), address(spNFT));
